...
This package mainly provides an numerical approach to derive the maximum likelihood estimation (MLE) of a multivariate folded normal distribution (FN). 
In addition, we also provide the estimated coverage probability of the 95% confidence intervals for the parameters.

## Definition of a multivariate FN distribution
...
$$f_{\bf X}(\bf x; \bf \mu, \bf \Sigma)=\sum_{\bf s \in \bf S(n)} (2 \pi)^{-\frac{n}{2}} |{\bf \Sigma}|^{-\frac{1}{2}}\exp\left\{-\frac{1}{2}(\bf\Lambda_{\bf s}^{(n)}{\bf x} - {\bf\mu})^{'} {\bf\Sigma}^{-1}(\bf\Lambda_{\bf s}^{(n)}\bf x - {\bf\mu})\right\},  \  {\bf x \geq \bf 0},$$
where 
...
The MLE of a multivariate FN is calculated by the `FN_MLE` function and the estimated covariance matrix is derived by the `est_helper.G2` function.

The output of `FN_MLE` function is:


| output |  explanation|
|:--------|------:|
|par | The estimated MLEs of parameters|
|value | the value log-likelihood function|
|counts | function??? gradient ???|
|convergence | 0: converage; ???|
|message | NULL |
|hessian | The hessian matrix of parameters|
...
### Input Data Examples: a bivarate FN distribution
...
Thus the example input data is
...
Please vist the [FNMLE-emo](https://github.com/yfyang86/FNMLE) project for more demos.
